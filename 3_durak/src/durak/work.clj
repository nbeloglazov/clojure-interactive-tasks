(ns durak.work
  (:require [durak.core :refer [run-game]]))


;;; Your task is to write a bot for playing card game "Durak": http://en.wikipedia.org/wiki/Durak
;;; Your bot plays against another built-in bot (2 players).
...
;;; Note that if it is not the first card in current attack, it must have the same rank as one of the cards on the table (check rules).
...
;;; If you return nil it means you can't (or don't want to) defend and you take all cards from the table to you hand.
...
;;; 13 - King
;;; 14 - Ace
;;; Suit is one of the 4 keywords: :spades, :hearts, :diamonds or :clubs
...
;;; If you are an attacker and it's the start of the attack then table will be empty.
;;; If you are a defender then table will be non-empty and your need to beat last card from the table.
;;; Cards on the table can be represented like: attack  - defense - attack - defense - attack. Odd cards are attack cards, even cards are defense cards.
;;; Trump is trump suit of the game.

;;; To test your solution use run-game function: (run-game YOUR_SOLUTION)
;;; Your bot is the player in the bottom part of screen (cards a visible to your).
...
;;; Implement program that takes 2 bots, runs game and returns winner.
...
;;; Use clojure atoms or refs to store data between moves.
...
;;; Modify program so it 3 or 4 players can play at the same table.
